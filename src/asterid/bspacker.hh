...
namespace asterid {
	...
	template <typename T, typename Q = unsigned int> class bspacker {
		
	public:
		
		bspacker(Q init_width, Q init_height) {
			root = new bspacker_node(init_width, init_height);
		}
		...
		bsp_coords pack (T * v, Q width, Q height) {
			...
			if (!ok) { // expand
				if (root->width + width > root->height + height) { // expand by height
					bspacker_node * new_root = new bspacker_node {std::max(root->width, width), root->height + height};
					new_root->child_a = root;
					new_root->child_b = new bspacker_node {nullptr, std::max(root->width, width), height, 0, root->height};
					new_root->pack_iter(ok, v, width, height, c);
					// assert(ok);
					root = new_root;
				} else { // expand by width
					bspacker_node * new_root = new bspacker_node {root->width + width, std::max(root->height, height)};
					new_root->child_a = root;
					new_root->child_b = new bspacker_node {nullptr, width, std::max(root->height, height), root->width, 0};
					new_root->pack_iter(ok, v, width, height, c);
					// assert(ok);
					root = new_root;
				}
	...
	private:
		...
		struct bspacker_node {
			...
			bspacker_node(T const * v, Q width, Q height, Q x, Q y) : width(width), height(height), coords(x, y), value(v) {}
			bspacker_node(T const * v, Q width, Q height) : bspacker_node(v, width, height, 0, 0) {}
			bspacker_node(Q width, Q height) : bspacker_node(nullptr, width, height, 0, 0) {}
			...
			void pack_iter (bool & ok, T const * const & v, Q const & width, Q const & height, bsp_coords & pos) {
				...
				if (width != this->width && height != this->height) { // children won't be enough, one of the children needs to have further children
					child_a = new bspacker_node { nullptr, width, this->height, coords.x, coords.y };
					child_b = new bspacker_node { nullptr, this->width - width, this->height, coords.x + width, coords.y };
					child_a->pack_iter(ok, v, width, height, pos);
					// assert(ok);
					return;
				}
				...
				if (width == this->width) { // width matches, single split
					child_a = new bspacker_node { v, width, height, coords.x, coords.y };
					child_b = new bspacker_node { nullptr, width, this->height - height, coords.x, coords.y + height };
					ok = true;
					pos = child_a->coords;
					return;
				}
